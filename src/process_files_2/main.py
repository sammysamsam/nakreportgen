...
def master_parser_1(files, debug=True):
        file_interest = None

        # exponential big O but not expecting large # of files
        for file in files:
            file_ = file.split("\\")[-1].replace(" ", "").lower().replace(".", "").replace("avenue", "ave").replace(
        ...
        if file_interest is None:
            for file in files:
                file_ = file.split("\\")[-1].replace(" ", "").lower().replace("acp5", "").replace(".", "").replace(
                    "avenue", "ave").replace("(1)", "").replace("(2)", "").replace("(3)", "")
                if "stpdf" in file_:
                    for file2 in files:
                        file2_ = file2.split("\\")[-1].replace(" ", "").lower().replace("acp5", "").replace("asp5",
                                                                                                            "").replace(
                            ".", "").replace("avenue", "ave").replace("(1)", "").replace("(2)", "").replace("(3)", "")
                        if file2_ == file_:
                            continue
        ...
        if file_interest is not None:
            ...
            try:
                r = parse_data_1(file_interest)
                return r
            except Exception as e:
                if debug:
                    raise e
                pass
def master_parser_2(files, debug=True):
...
def master_writer(result, working_dir, debug=True):
    try:
        ...
        load_template.load_template(join(HOME_DIR, "TEMPLATE2.docx"))
        ...
        load_template.update_temp("A", result.get("a"))
        load_template.update_temp("B", result.get("b"))
        load_template.update_temp("C", result.get("c"))
        load_template.update_temp("D", result.get("d"))
        load_template.update_temp("squareft", result.get("sq"))
        load_template.update_temp("linearft", result.get("lin"))

        load_template.update_table_three_columns("table1", result.get("tabledata"))
        load_template.update_table("table2", result.get("tabledata"))
        load_template.update_table_yes("table3", result.get("tabledata"))
        load_template.update_table_three_columns("table4", result.get("tabledata"))
        load_template.update_table("table5", result.get("tabledata"))
...
def main(debug=True):
    ...
    for working_dir in allworkingdirectories.keys():
        ...
        r = master_parser_1(allworkingdirectories[working_dir], debug=debug)
        if r is not None:
            result.update(r)
            partA_done = True

        r = master_parser_2(allworkingdirectories[working_dir], debug=debug)
        if r is not None:
            result.update(r)
            partB_done = True
...
if __name__ == '__main__':
    ...
    DATA_DIR = join(HOME_DIR, "Files_2")
